...
const userServices = {
  ...
  updateFriend: async (query, updateObj) => {
    return await friendsModel.findOneAndUpdate(query, updateObj, { new: true });
  },
            ...
            $reduce: {
              input: {
                $filter: {
                  ...
                  as: "account",
                  cond: {
                    $eq: ["$$account.type", "current"],
                  },
                },
              },
              initialValue: "",
              in: { $concat: ["$$value", { $toString: "$$this.amount" }] },
            },
          },
          savingAmount: {
            $reduce: {
  ...
  aggregateSearchUser: async (validatedBody) => {
    try {
      const { search, fromDate, toDate, page, limit, status1, approveStatus } =
        validatedBody;
      if (approveStatus) {
